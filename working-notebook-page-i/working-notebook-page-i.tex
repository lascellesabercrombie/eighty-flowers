...
\usepackage{tikz}
\usepackage[paperwidth=5.3in, paperheight=15.5in, top=0.5in, bottom=0.1in, left=0.5in, right=0.5in]{geometry}
...
\newcommand{\buttercupclematisarrow} {
	\color{black}
	\begin{tikzpicture}[remember picture,overlay]
		\draw[->] (0,0) -- (0.5,0.1);
	\end{tikzpicture}
...
\item\color{red}\biggerbrace\color{black}cactus$\color{red}\marginalampersand\color{blue}\smallertick$ \footnotesize(``rose-mauve$\color{blue}\smallertick$ fluted'' \& P's)\par
\tiny
\color{blue}
...
note$\color{black}\smallcross$ \ul{Weeds} 
\setulcolor{blue}
...
\setulcolor{black}
\color{red}
Field Flowers p9 illust \color{red}belle looking glass\par
\normalsize
\color{black}
\item $\smallbrace$clematis (\ul{Crispa} \color{blue}= \color{black} ``blue jasmine'' \color{blue}= \color{black} ``bluebell''\par
...
\begin{textblock*}{4in}(0cm,26cm)%
	...
	\begin{minipage}{4in} 
		\color{blue}
		buttercup$\buttercupclematisarrow$\par
...
\begin{textblock*}{8cm}(0.3cm,27.4cm)%
	...
	\begin{minipage}{8cm} 
		...
		\setulcolor{red}
		\color{red}
		\& var. Claytonia virginica using \ul{jackmani}\par
...
\begin{hangparas}{2em}{1}
	Botanical notes taken during trips to Bermuda (Paget) and Bellagio (Lake Como, Italy) in 1972 (notes are at the back of the "A"-22 \& -23 notebook HRC 37.4)\par
	\textit{Taylor's Encyclopedia of Gardening} - Agapanthus (15), Trailing Arbutus (1219), Azalea (88-89), Campanula (182-184), Specularia (1135), Clematis (236), Claytonia (235)\par
	Everett, \textit{A Guide to Wild Flowers: Woodland Flowers} - entries for Wood Anemone (Windflower) (56), Spring Beauty (Claytonia virginica) (45)\par
	Everett, \textit{A Guide to Wild Flowers: Field Flowers} - entry for Venus's-Looking-Glass (9)\par
\end{hangparas}
}
